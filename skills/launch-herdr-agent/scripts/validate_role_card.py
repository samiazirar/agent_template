...
CONTRACTS = {
    ...
    "operations-lead": (
        ...
        "DISPATCH RULE: Send an already-atomic package directly to a native-Codex Sol-medium worker. For a meaningful multi-package task, create one native-Codex Sol-medium suborchestrator; only that suborchestrator may launch Luna-max workers.",
    ...
    "suborchestrator": (
        ...
        "ROUTING RULE: Keep one current next package, launch one fresh native-Codex Luna-max worker for normal coding or native-Codex Sol-medium worker for a harder package, arm herdr-emergency-wake instead of waiting, close each worker after its wake message, absorb results without routine review turns, integrate the task, report with herdr-role-message operations, then stop.",
