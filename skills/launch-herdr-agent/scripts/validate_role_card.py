...
CONTRACTS = {
    ...
    "worker": (
        "ROLE: Worker",
        "MODEL: GPT-5.6 Luna low by default; Sol medium after bounded-task escalation; explicitly selected Opus 5 or GLM 5.2 alternative",
        "RECEIVES FROM: Operations Lead or one owning suborchestrator",
        "SENDS TO: The same assigning role",
        "OWNS: One clear repeatable subtask; one deliverable; one reproduction or run; one done check; one branch; one worktree; one result",
        "MUST NOT: Contact the Human or Human Orchestrator; edit HUMAN_PLAN.md; launch agents; broaden scope; perform unrelated work",
        "ROUTING RULE: Self-verify the attached done check, report to Operations Lead with herdr-role-message operations or to one owning suborchestrator with herdr-role-message named, then stop. Escalate material ambiguity, one failed coherent repair, scope expansion, contradictory tool results, or a non-reproducible result.",
    ),
    "suborchestrator": (
        ...
        "OWNS: One independent multi-step workstream expected to contain at least three Luna-sized productive pieces",
        "MUST NOT: Contact the Human or Human Orchestrator; create another suborchestrator; manage work outside its workstream",
        "ROUTING RULE: Issue only the next useful bounded pieces, launch one Luna worker per piece, absorb compact results without routine review turns, integrate the workstream, report with herdr-role-message operations, then stop.",
    ),
    "strategic-advisor": (
    ...
    "researcher": (
        "ROLE: Researcher",
        "MODEL: GPT-5.6 Terra medium for open-ended synthesis; GPT-5.6 Luna low for routine source finding and transformation",
        ...
        "ROUTING RULE: Use Luna for clear repeatable source work and Terra for open-ended synthesis, return the bounded result to the assigning role, then stop.",
    ),
    "research-observer": (
