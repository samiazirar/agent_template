...
CONTRACTS = {
    ...
    "operations-lead": (
        ...
        "DISPATCH RULE: Default every meaningful productive task to one quiet native-Codex Sol-medium suborchestrator; it launches a new Luna-max session for each frozen atomic package. Launch Luna-max directly only for an explicit tiny atomic task. Set HERDR_TASK_CARD on guarded herdr-agent launches. Use a Sol-medium coding worker only with a recorded Luna failure; non-Luna alternatives require the Human's explicit model choice.",
